  ...
  return (
    <div
      style={{
        width: size,
        height: size * 0.92,
        background: `radial-gradient(circle at 35% 30%, ${palette.blobGoo}, ${hue} 55%, ${palette.blobDeep} 100%)`,
        borderRadius: "48% 52% 50% 50% / 55% 50% 50% 45%",
        position: "relative",
        boxShadow: `inset 0 -${size * 0.1}px ${size * 0.1}px ${palette.blobDeep}, 0 ${size * 0.06}px 0 rgba(0,0,0,0.35)`,
      }}
    >
      {eyes && (
        <>
          <Eye left={size * 0.25} top={size * 0.28} d={size * 0.22} />
          <Eye left={size * 0.55} top={size * 0.28} d={size * 0.22} />
          <div
            style={{
              position: "absolute",
              left: size * 0.32,
...
function Eye({ left, top, d }: { left: number; top: number; d: number }) {
  return (
    <div
      style={{
        position: "absolute",
        left, top, width: d, height: d,
        background: "#fff",
        borderRadius: "50%",
